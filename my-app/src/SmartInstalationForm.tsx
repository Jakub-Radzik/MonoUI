import { Form, InputNumber, DatePicker, Switch, Button, Card, Tag, Select, Radio } from "antd";
...
const SmartInstallationForm = () => {
  ...
  useEffect(() => {
    const updateView = async () => {
      ...
      const result = await callApiAlgorithm(payload, location, viewMode === "monthly");
      setData(result);
    };
  ...
  const formattedData = data.map((item) => ({
    ...
    formattedDate:
      viewMode === "monthly"
        ? new Date(item.date).toLocaleDateString("pl-PL", { month: "long", year: "numeric" })
    ...
    <Card title="Kalkulator zużycia" style={{ maxWidth: 900, margin: "auto" }}>
      <Form form={form} layout="vertical" onFinish={onFinish} initialValues={initForm}>
        ...
        <Form.Item
          ...
          name="realPower"
          rules={[{ required: true, message: "Moc instalacji jest wymagana" }]}>
            ...
            <BarChart data={formattedData}>
              ...
              <Tooltip
                ...
                labelFormatter={(label: string) =>
                  viewMode === "daily" ? `Data: ${label}` : `Miesiąc: ${label}`
                }
              />
              ...
              <Bar radius={[6, 6, 0, 0]} dataKey="usage" fill="url(#barGradient)">
